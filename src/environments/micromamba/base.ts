...
import { updateEnvironmentsTxt } from '../condaHelper';
...
export async function createBaseEnv(progress: Progress<{
    message?: string | undefined;
    increment?: number | undefined;
}>, token: CancellationToken, refreshUntilAvailable: RefreshUntilNewEnvIsAvailable, pythonVersion = '3.9') {
    try {
        ...
        const args = ['install', `python=${pythonVersion || '3.9'}`, 'conda', '-c', 'conda-forge', '-y', '-p', MICROMAMBA_ROOTPREFIX];
        ...
        if (!(await fs.pathExists(BASE_MICROMAMBA_PYTHON_EXE))) {
            throw new Error(`Please try running the following command in the terminal "${[MICROMAMBA_EXE].concat(args).join(' ')}"`);
        }
        await updateEnvironmentsTxt(MICROMAMBA_ROOTPREFIX).catch(ex => traceError('Failed to update environments.txt', ex));
        ...
        window.showErrorMessage(`Failed to create environment ${MICROMAMBA_ROOTPREFIX}, ${ex.toString()}`);
    }

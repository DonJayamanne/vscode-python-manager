...
export async function deleteEnv(env: Environment | ResolvedEnvironment) {
    ...
    if (!baseDir.toLowerCase().endsWith('scripts') && !baseDir.toLowerCase().endsWith('bin')) {
        traceError(
            `Cannot delete as its not a virtual environment with script/bin directory ${getEnvLoggingInfo(env)}`,
        );
        return;
    }
    const dirToDelete = env.environment?.folderUri?.fsPath || path.dirname(path.dirname(env.path));
    traceVerbose(`Deleting virtual environment ${getEnvLoggingInfo(env)}`);
    await fs.remove(dirToDelete);
}

function getSortedEnvsFromWhichWeCanCreateVenvEnv(environments: readonly Environment[]) {
...
export function canCreateVirtualEnv(environments: ResolvedEnvironment[]) {
    return getSortedEnvsFromWhichWeCanCreateVenvEnv(environments).length;
}
export async function createEnvOld() {
    ...
    const latestGlobal = templateEnvs.filter(
        (env) => getEnvironmentType(env) === EnvironmentType.Global,
    );

    type StateType = { dir: string; name: string; templateEnvironment?: ResolvedEnvironment };
    ...
    const selectTemplateEnv = async (
        input: MultiStepInput<StateType>,
        state: StateType,
    ): Promise<InputStep<StateType> | void> => {
        type QuickPickItemWithEnvironment = QuickPickItem & { pythonEnvironment: ResolvedEnvironment };
        const quickPickItems = templateEnvs.map(
            (env) =>
                <QuickPickItemWithEnvironment>{
                    label: getEnvDisplayInfo(env),
                    pythonEnvironment: env,
                    picked: env === templateEnvironment,
                    description: getDisplayPath(env.path),
                },
        );
        ...
        while (selectDirectory) {
            ...
            if (directory === browseButton) {
                ...
                if (Array.isArray(dir) && dir.length) {
                    ...
                    if (await fs.pathExists(targetVenvDir)) {
                        void window.showErrorMessage('A folder with the same name already exists', {
                            modal: true,
                ...
                if (!(await fs.pathExists(directory.trim()))) {
                    void window.showErrorMessage('Invalid target directory for a Virtual Environment', {
                        modal: true,
                        detail: directory,
                    });
    ...
    try {
        await window.withProgress(
            ...
            async (
                ...
                const result = await execObservable(
                    initialState.templateEnvironment!.path,
                    ['-m', 'venv', initialState.name],
                    {
                        timeout: 60_000,
        ...
        void commands.executeCommand('python.envManager.refresh', false);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (ex) {
        traceError(`Failed to create environment`, ex);
        window.showErrorMessage(`Failed to create environment ${initialState.name}, ${ex}`);
    }
}
